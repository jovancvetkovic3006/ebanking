  ...
  return (
    <div className="max-w-5xl mx-auto p-6 space-y-6">
      ...
      <div className="flex items-center justify-between">
        <Link className="underline disabled:opacity-50" aria-disabled={page === 1} href={`/transactions?page=${prev}`}>
          Previous
        </Link>
        <div className="text-sm">Page {page} of {totalPages}</div>
        <Link className="underline disabled:opacity-50" aria-disabled={page === totalPages} href={`/transactions?page=${next}`}>
          Next
        </Link>
      </div>
    </div>
